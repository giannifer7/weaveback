...
1. link:https://giannifer7.github.io/weaveback/crates/weaveback-macro/src/weaveback_macro.html[*weaveback-macro*] — expands `%macro(...)` calls
2. link:https://giannifer7.github.io/weaveback/crates/weaveback-tangle/src/weaveback_tangle.html[*weaveback-tangle*] — extracts `<[@file ...]>` chunks and writes them to disk
...
`weaveback serve` starts a local HTTP server with live-reload, an inline chunk
editor, and an AI assistant panel that uses your Claude Code session — no
separate API key required.

image::Screenshot_20260328_140347.png[AI assistant panel showing contextual explanation of a chunk,width=700]

The assistant receives the full chunk context: source body, surrounding prose
and design notes from the `.adoc`, dependency bodies, and recent git history.

---

== Agent and IDE integration

`weaveback mcp` starts an MCP server over stdin/stdout, exposing six tools
...
- `weaveback_chunk_context` — full context for a chunk (prose, deps, git log)
- `weaveback_list_chunks` — enumerate all chunks in the project
- `weaveback_find_chunk` — locate which file defines a chunk name
...
See link:docs/tracing.adoc[Source tracing] for the full workflow, gotchas, and
