// crates/azadi/tests/test_trace.rs
//
// Integration tests for `azadi where` and `azadi trace`.
...
/// Run azadi on a single driver file, returning the canonicalized temp directory.
fn run_azadi(tmp: &TempDir, source: &str) -> std::path::PathBuf {
...
/// Run `azadi where <out_file> <line>` from `dir` and parse JSON output.
...
fn trace_json(dir: &Path, out_file: &str, line: u32) -> Value {
    ...
    serde_json::from_slice(&out.stdout)
        .unwrap_or_else(|e| panic!("bad JSON from 'azadi trace': {e}\nstdout: {}", String::from_utf8_lossy(&out.stdout)))
}

// ── Correctness: azadi where ──────────────────────────────────────────────────

/// Plain literal line: `azadi where` returns the correct chunk, file, and line.
#[test]
...
/// `azadi trace` always returns macro-level fields (tracing is unconditional).
#[test]
...
/// Time a single azadi run; returns elapsed milliseconds.
fn time_run(source: &str) -> u128 {
    ...
    let mut cmd = std::process::Command::new(cargo_bin("azadi"));
    cmd.arg("--gen").arg(".");
