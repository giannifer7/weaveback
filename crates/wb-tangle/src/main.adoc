...
`wb-tangle` is the weaveback build tool.  It operates in two modes:

* **Multi-pass** (default): reads `weaveback.toml` and runs all `[[pass]]`
  entries in parallel, then rebuilds FTS.
...
The CLI is split into two modes via a clap `command` dispatch: when `--dir` is
provided the single-pass sub-command fires; otherwise the multi-pass default
runs.
