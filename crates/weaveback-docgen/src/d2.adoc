...
Walk the parsed AsciiDoc document and collect all D2 raw blocks, accepting both
`[d2]` and `[source,d2]`, and returning their byte offsets and source text.
...
If the `d2` binary is unavailable, D2 preprocessing is skipped for that file
and the original source is left intact. Missing tooling should not make docs
rendering fail in CI; actual D2 render failures still propagate as errors.
