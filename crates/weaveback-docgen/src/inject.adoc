...
The inline unit tests belong to the same source of truth as the production
logic.  Keeping them in the same literate source avoids stale trailing test
modules in the generated `.rs` file and keeps coverage attribution aligned with
the real owning document.
