...
`nested_blocks()`.  For each `Block::RawDelimited` whose first positional
attribute (block style) is `"plantuml"`, we record:
...
`render_diagram` pipes `diagram_source` to `java -jar <jar> -tsvg -pipe` and
returns the SVG bytes.  PlantUML writes the SVG to stdout and exits 0 on
success.
...
`batch_render_plantuml` renders all supplied diagrams in one `java -jar`
call, paying JVM startup cost exactly once regardless of diagram count.
