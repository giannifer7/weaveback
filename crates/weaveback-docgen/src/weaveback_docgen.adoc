...
The tool is generic: it works for any project that uses asciidoctor + weaveback.
...
| Path to `plantuml.jar`.  When given, `[plantuml]` blocks are rendered by
  calling `java -jar <path>` directly; `asciidoctor-diagram` is not required.
...
`Args` collects all CLI options.  `parse_args` walks `std::env::args()` manually
