...
/// Scan `.adoc` files in `crates_dir` for `<<@file path>>=` declarations.
/// Returns a map from adoc-derived HTML path (relative to project root,
/// forward-slash separated) to the list of module keys declared in that file.
///
/// Example: `crates/weaveback-macro/src/bin/cli.adoc` declares
/// `<<@file weaveback-macro/src/bin/weaveback-macro.rs>>=`
/// → entry `"crates/weaveback-macro/src/bin/cli.html"` → `["weaveback_macro/bin/weaveback-macro"]`
pub fn scan_adoc_file_declarations(
    ...
    for adoc in adoc_files {
        ...
        for cap in re.captures_iter(&content) {
            ...
            // @file paths are relative to crates_dir (e.g. "weaveback-macro/src/bin/foo.rs")
