...
* **`test_serialize_token_src_field_present`** — `token.src` must appear
  ...
  came from.
* **`test_strip_*`** — cover the five cases of `strip_space_before_comments`:
  Space node removed, Text node trimmed, block comment followed by newline
  (stripped), block comment not followed by newline (kept), and multiple
  consecutive Space nodes all removed before a comment.
