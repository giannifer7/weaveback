...
A single forward scan accumulates state flags (`seen_equal`,
`param_name`, `first_good_after_equal`) and then a second loop converts
children starting at the computed `start_idx`.  This avoids backtracking
while keeping the code readable.

=== Why `NOT_FOUND = -1`?

`analyze_param` tracks several "first seen" positions as `i32` with `-1`
as sentinel.  Using `i32` throughout the scan loop avoids constant
wrapping/unwrapping of `Option<usize>`, and the comparisons read as
straightforward index arithmetic.
...
Each node is encoded as a JSON-like array:
