...
sets `early_exit` so the evaluator stops cleanly.  This is intentionally
one-shot: re-running on the already-patched file is a no-op because `%%here`
(double special char) is literal text, not a macro call.
