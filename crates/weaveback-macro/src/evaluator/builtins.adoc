...
* **Start**: `[A-Za-z_]` — the lexer never emits an `Ident` token whose first
  byte is a digit or a special character.
...
Hyphens are `Special` tokens in the lexer, so `foo-bar` would lex as three
separate tokens (`foo`, `-`, `bar`) and the "exactly one non-space child" check
would fire.
