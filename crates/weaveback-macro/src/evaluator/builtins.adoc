...
== `define_macro` — shared helper for `%def` / `%redef` / `%rhaidef` / `%pydef`
...
`builtin_importas` also runs the file like `%import`, but additionally
registers every newly-defined macro under a `prefix_name` alias.  The
originals remain so that internal cross-references inside the file continue
to resolve.  The prefix must be a valid identifier.
