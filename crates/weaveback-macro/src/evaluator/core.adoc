...
Built-in macro calls are delegated to the plain `evaluate_macro_call` and the
result is pushed as `push_untracked`.  User-defined macros go through the full
parameter-binding and tracing machinery.  The body is evaluated with a
`MacroBody` context span; script-kind macros use the same note as in the plain
path — they arrive through the builtin map, so the `ScriptKind::Rhai` /
`ScriptKind::Python` branches here are currently unreachable.
