...
1. A new `ScopeFrame` is pushed onto the scope stack.
2. Each argument node is evaluated left-to-right *inside that new frame*,
   producing a `String`; the string is then bound to the corresponding
   formal parameter in the same frame.
3. The macro body is evaluated inside the frame.
4. The frame is popped.
5. For `%rhaidef`/`%pydef` macros, the _already-expanded_ body string is then
   passed to the Rhai or Python engine; there is no lazy re-expansion.
...
the _callee's_ scope frame, not the caller's.  Those mutations disappear when
...
* **Argument side-effects are scoped.**  `%set` inside an argument lives in
  the callee's frame and does not bleed into the caller (see above).
* **Recursion depth limit.**  A macro calling itself (directly or indirectly)
  will hit `MAX_RECURSION_DEPTH = 100` and return a runtime error rather than
  stack-overflow.
