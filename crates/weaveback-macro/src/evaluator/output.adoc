...
- `push_untracked` — no span; used for Rhai results and builtins whose output can't be attributed to a single source location. It still advances the line counter by scanning for `\n` bytes, keeping `line_spans` in sync with the actual output.
