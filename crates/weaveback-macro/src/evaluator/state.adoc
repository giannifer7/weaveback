...
class EvaluatorState {
  ...
  +call_depth: usize
  +early_exit: bool
  +discovered_includes: Vec<PathBuf>
...
`discovery_mode` is a special flag used by the directory-scan driver: the
`%include` / `%import` argument is still evaluated normally, but the evaluator
does not recurse into the target file.  Instead it records the resolved path in
`discovered_includes` and returns.  This preserves conditional include idioms
while still collecting dependencies.
