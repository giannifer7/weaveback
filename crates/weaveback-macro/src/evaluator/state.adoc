...
* **Empty spans** — untracked (Rhai/Python script result).  The value is
...
`Rhai` / `Python`:: Body is macro-expanded first, then the result string is
  executed as a script.  All visible scope variables are injected into the
  script environment.
`RhaiRaw` / `PythonRaw`:: Body is passed as literal script source (no macro
  expansion).  Only the declared parameters are injected as script variables.
