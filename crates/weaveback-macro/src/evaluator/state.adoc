...
* **Empty spans** — untracked (Rhai/Python script result).  The value is
  ...
  information.  Built-in macro results are no longer in this category: they
  are pushed with `SpanKind::Computed` pointing to the call site (see
  `evaluate_macro_call_to` in `core.rs`).
