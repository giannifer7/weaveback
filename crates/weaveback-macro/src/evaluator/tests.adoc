...
* link:tests-scripting.adoc[tests-scripting.adoc] — Rhai (`%rhaidef`, `%rhaiset`, …) and Python (`%pydef`, `%pyset`, …) scripting + engine unit tests
...
| `test_importas`
| `%importas(prefix, path)`: prefixed aliases, multiple macros, originals
  remain, cross-reference preservation, arity/identifier guards, discard text

| `test_raw_scripts`
| `%rhaidef_raw` / `%pydef_raw`: raw body (no macro expansion), param injection,
  contrast with non-raw, arity/name guards
...
// <<@file weaveback-macro/src/evaluator/tests/test_rhai_eval.rs>>=
// <<test rhai eval>>
// @
----
