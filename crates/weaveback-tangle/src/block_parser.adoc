...
* **AsciiDoc** (`.adoc`, `.asciidoc`) — uses `asciidoc-parser` for accurate
  block detection, with a `catch_unwind` fallback to a simple line scanner if
  the parser panics.
...
The AsciiDoc scanner uses `asciidoc-parser` to walk the block tree:
...
* `Block::RawDelimited` → `"code"` with the full span (including delimiters).
* Everything else → `"para"` with the full span.

Because `asciidoc-parser` is an experimental pre-release crate, the call is
wrapped in `catch_unwind`.  If the parser panics the function falls back to
`parse_adoc_raw_simple` — a conservative line scanner that splits on
`----`/`....`/`++++` fences and `== …` section headers.  The fallback is kept
as dead code unless a panic is encountered; it does not affect normal operation.
