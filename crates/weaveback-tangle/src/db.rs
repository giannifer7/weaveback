// src/db.rs  —  weaveback persistent database (SQLite, WAL mode)
//
// Tables:
...
//
// Concurrency model:
//   weaveback runs write to an in-memory temp db (WeavebackDb::open_temp), then
...
impl WeavebackDb {
    ...
    /// Use this for the target `weaveback.db` in read-write contexts.
    ...
    /// Create an in-memory database for use as a write buffer during an weaveback
    ...
    pub fn merge_into(&self, target_path: &Path) -> Result<(), DbError> {
        ...
        // Set a generous busy_timeout so concurrent weaveback processes retry
