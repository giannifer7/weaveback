...
NOTE: This remap is *best-effort*.  Two known lossy cases:

* Lines inserted before any existing content (e.g. a copyright header added by
  the formatter) have no preceding mapped line and receive no attribution.
* Large-scale reformats (e.g. a formatter that sorts or merges blocks) produce
  a diff where almost all lines are `Insert`/`Delete`; the attribution will be
