...
The remap uses a three-tier strategy, applied in order:

. *Diff anchor*: `similar::TextDiff` maps `Equal` lines exactly from pre- to
  post-formatter positions.  This is the primary, precise path.
. *Content hash fallback*: for new lines still unattributed after the diff
  (e.g. lines reordered by import sorting), compute a normalised hash of each
  pre-formatter line and match it against unmatched post-formatter lines by
  content.  Normalisation strips leading/trailing whitespace and inline
  `//`-style comments so that spacing-only changes do not defeat the match.
. *Bidirectional nearest-neighbour fill*: any remaining gaps are filled by the
  nearest attributed line, scanning forward first (prefer the preceding source
  line) then backward (covers leading insertions such as copyright headers that
  the formatter prepends before any original content).

NOTE: The remap is *best-effort*.  Attribution is approximate when a formatter
makes large-scale semantic changes (e.g. merges or splits blocks).  The hash
fallback handles structure-preserving reorders (import sorting, declaration
reordering); beyond that, the nearest-neighbour fill gives a reasonable
approximation.
