...
. `after_write(name)` — runs the optional formatter on the temp copy, checks
  ...
  temp file to `gen/` if the content changed, and stores the new content as
  the baseline in the in-memory database.
...
`run_formatter` shells out to the configured formatter command, splitting the
command string on whitespace and appending the temp-file path as a final
argument.
