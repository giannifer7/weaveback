...
. For each changed line in a `Replace` hunk of equal size:
  ...
  .. Optionally call `perform_trace` (link:lookup.adoc[`lookup.rs`]) for
     macro-level attribution.
  .. Classify the attribution as one of the `PatchSource` variants.
...
=== Heuristic patch + oracle

For `MacroBodyWithVars` and `MacroArg` patches, a candidate source edit is
generated heuristically and then *verified* by re-evaluating the patched
source with the macro expander and confirming the relevant expanded line
matches the desired output.  A wrong candidate simply fails the oracle check
and is not written.

=== Fuzzy line matching

When the source line is not found at the expected index (e.g., after an
unrelated edit above it), a ±15-line window search using a
whitespace-normalised regex is tried.
