...
`lint.rs` is the beginning of a structural linter for literate sources.
The point is not generic style enforcement.  The point is to make project
invariants explicit and cheap to check.

The first implemented rule is `chunk-body-outside-fence`.

== Rule model and directory scanning
