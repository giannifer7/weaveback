...
fn main() {
    ...
    let result = match cli.command {
        ...
        Some(Commands::Coverage { summary, lcov_file }) => {
            let eval_config = build_eval_config(&cli.args);
            run_coverage(summary, lcov_file, cli.args.db, cli.args.gen_dir, eval_config)
        }
...
fn print_coverage_summary(summary: &serde_json::Value) {
    println!(
    ...
    if let Some(sources) = summary["sources"].as_array() {
        for source in sources.iter().take(10) {
            ...
            println!("{src_file}: {covered}/{total} covered, {missed} missed");
            if let Some(sections) = source["sections"].as_array() {
                for section in sections.iter().take(3) {
                    ...
                    println!("  {breadcrumb}: {covered}/{total} covered, {missed} missed");
                }
            }
        }
    }
    ...
    if unattributed > 0 {
        println!("Unattributed line records: {unattributed}");
    }
}
...
fn run_coverage(
    ...
    let summary = build_coverage_summary(&records, &db, &project_root, &resolver, &eval_config);
    if summary_only {
...
mod tests {
    ...
    fn coverage_command_options_support_summary() {
        ...
        .unwrap();
        match cli.command.unwrap() {
            Commands::Coverage { summary, lcov_file } => {
                assert!(summary);
