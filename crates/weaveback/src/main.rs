...
/// Compute the unmapped line ranges from a file's `generated_lines` array
/// (lines that had coverage data but no noweb entry).  Groups consecutive
/// line numbers into `(start, end, missed_count)` tuples.
fn unmapped_ranges(file: &serde_json::Value) -> Vec<(u64, u64, usize)> {
    let mut lines: Vec<(u64, bool)> = file["generated_lines"]
        .as_array()
        .map(|arr| {
            arr.iter()
                .filter_map(|r| {
    ...
    for (ln, hit) in lines {
            ...
            && ln == last.1 + 1
        {
            ...
            ranges.push((ln, ln, if missed { 1 } else { 0 }));
        }
    }
    ranges
}

/// Print unmapped line ranges for one unattributed file, with source content
/// when the file can be read from disk.
fn explain_unattributed_file(
    file: &serde_json::Value,
    out: &mut impl Write,
) -> std::io::Result<()> {
    let ranges = unmapped_ranges(file);
    if ranges.is_empty() {
        return Ok(());
    }
    ...
    let file_lines: Option<Vec<String>> = std::fs::read_to_string(path)
        .ok()
        .map(|s| s.lines().map(str::to_owned).collect());

    for (start, end, missed) in &ranges {
        ...
        if let Some(files) = summary["unattributed_files"].as_array() {
            for file in files.iter().take(top_sources) {
                ...
                if explain_unattributed {
                    explain_unattributed_file(file, &mut out)?;
                }
            }
        }
    }
...
fn run_coverage(
    ...
    if summary_only || explain_unattributed {
