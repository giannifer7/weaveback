...
fn run_impact(chunk: String, db_path: PathBuf) -> Result<(), Error> {
    let json = weaveback_api::query::impact_analysis(&chunk, &db_path)?;
    println!("{}", serde_json::to_string_pretty(&json).unwrap());
    Ok(())
}

fn run_graph(chunk: Option<String>, db_path: PathBuf) -> Result<(), Error> {
    let dot = weaveback_api::query::chunk_graph_dot(chunk.as_deref(), &db_path)?;
    println!("{dot}");
    Ok(())
}
...
fn run_tags(file: Option<String>, db_path: PathBuf) -> Result<(), Error> {
    let blocks = weaveback_api::query::list_block_tags(file.as_deref(), &db_path)?;
    if blocks.is_empty() {
