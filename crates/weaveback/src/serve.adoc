...
1. Run `weaveback serve` in one terminal.
...
`POST /__apply` edits a chunk body in the literate source file.
...
calls the Anthropic API with streaming enabled, and forwards the response
as Server-Sent Events back to the browser.  Requires the `ANTHROPIC_API_KEY`
