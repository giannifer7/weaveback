Here’s a design that I think fits **Weaveback** much better than changing prose language.
...
It preserves the prose as the main thing. The structured lines are just
anchors.
...
My advice: start with **freeform**, and only normalize where repeated
patterns emerge. Do not over-formalize too early.
...
These annotations apply to the section above unless a chunk immediately
follows and you define that adjacency binds to the chunk. You need one
clear rule and to keep it simple.
...
That is enough to be useful without becoming a second programming
language.
...
You already care about traceability. So treat annotations as first-class
mapped entities.
...
This layer gives you a small semantic index without killing the human
writing experience.
...
But YAML has a bad habit: it visually dominates the prose and encourages
schema creep.
...
You can still keep surrounding prose in whichever language helps you
think best.
...
1. a tiny EBNF for this annotation syntax,
   or 2. a Rust parser/data model for it that fits your current architecture.
