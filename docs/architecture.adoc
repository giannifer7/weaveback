...
LSPs (like `rust-analyzer`) with Weaveback's literate source maps. This allows
developers and coding agents to perform "Go to Definition" and "Find References"
queries on generated code and have the results mapped directly back to the
original `.adoc` or `.md` source.

[source,bash]
----
weaveback lsp definition gen/src/main.rs 100 8
weaveback lsp references gen/src/main.rs 100 8
----

=== How it works

1.  **LSP Query**: Weaveback spawns a background language server (currently
    Rust-only via `rust-analyzer`) and sends a standard LSP request for the
    requested position in the generated file.
2.  **Semantic Resolution**: The LSP server resolves the query using its full
    semantic knowledge of the project (types, traits, macros).
