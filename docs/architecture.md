...
| Tool | Description |
|------|-------------|
| `weaveback_trace` | Trace a generated file line/column to its literate source. |
| `weaveback_apply_fix` | **Preferred edit tool.** Replace a line or range in the literate source and oracle-verify it produces the expected output before writing. Supports single-line (`src_line`) and multi-line (`src_line` + `src_line_end` + `new_src_lines`) replacements. |
| `weaveback_apply_back` | Bulk baseline-reconciliation. Use only when `gen/` files have already been edited by hand. |
