...
* `weaveback-api` single-pass processing has been split at the generated
  Rust-file level.  `crates/weaveback-api/src/process.rs` is now a small facade
  and implementation files live under `crates/weaveback-api/src/process/`.
...
`crates/weaveback-api/src/process.rs` | 19 | Runtime split completed; generated tests still need concern-level splitting.
...
* split the next largest remaining generated Rust file after a fresh line-count
  audit
* likely candidates include `process/tests.rs`, `mcp.rs`, parser/lexer modules,
