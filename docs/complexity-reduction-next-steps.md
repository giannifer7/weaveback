...
* `weaveback-api` lint has been split at Rust-file level.
  `crates/weaveback-api/src/lint.rs` is now a small facade over configuration,
  filesystem scanning, lint rules, and the CLI runner under
  `crates/weaveback-api/src/lint/`.
* `weaveback-agent-core` read API has been split at Rust-file level.
...
* `weaveback-api` LCOV summary helpers have been split at Rust-file level.
  Generated-file lookup and unmapped-range grouping now live under
  `crates/weaveback-api/src/coverage/lcov/summary/`, leaving the summary file
  focused on aggregation.
* `weaveback-docgen` xref has been split at Rust-file level.
  `crates/weaveback-docgen/src/xref.rs` is now a small facade over module-key
  conversion, workspace discovery, Rust analysis, import resolution, adoc
  declaration scanning, exclusion rules, and graph building.
