...
- A `%pydef` body with a Python syntax error (reported with the monty
...
`%pydef` runs inside a sandboxed interpreter with no filesystem access,
no network access, and no subprocess spawning. `%pydef` uses
https://github.com/pydantic/monty[monty], a minimal, secure Python
...
| **Expressiveness** | High (Turing-complete natively) | Moderate core, high via Python
...
| **Security** | `system` by default | `%env` opt-in; monty sandboxed (no I/O)
|===
