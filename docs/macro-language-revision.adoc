...
so it covers `%pydef` and `%pydef_raw` as well.
...
The non-raw `%pydef` (macro-expand body first, then execute) remains for
cases that intentionally want macro preprocessing. New code should prefer the
`_raw` variant.
...
Use `%pydef_raw` for new scripts; reserve `%pydef` for code that needs macro
preprocessing of the body.
____
...
. Keep Python code in raw-script form (`%pydef_raw`).
...
* `%pydef_raw` raw-body script variant
...
* Multiple-`%here` error
* Possible deprecation of non-raw `%pydef` in docs
