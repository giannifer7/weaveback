...
Persistent script stores (`rhai_store`, `py_store`) are an advanced effect
...
so it covers `%rhaidef`, `%pydef`, `%rhaidef_raw`, and `%pydef_raw` as well.
...
`%rhaidef` and `%pydef` as escape hatches for computed logic.

=== Implemented

`%rhaidef_raw(name, [params,] body)` and `%pydef_raw(name, [params,] body)`:

* The body is stored as literal script source — no macro expansion occurs.
* Declared parameters are injected directly as script-level variables.
* `%rhaidef_raw` injects only declared params (not the full scope).
* `%pydef_raw` uses the same injection as `%pydef`.
* Note: the weaveback lexer still tokenises the body at definition time, so
  `%` must not appear in an unrecognised position inside the body.

The old `%rhaidef` / `%pydef` (macro-expand body first, then execute) remain
for backward compatibility.  New code should prefer the `_raw` variants.

=== Coding-model rule

____
Inside a script body, script syntax means script syntax.
Use `%rhaidef_raw` / `%pydef_raw` for new scripts; reserve the non-raw
variants for code that needs macro preprocessing of the body.
____
...
. Keep Rhai/Python code in raw-script form (`%rhaidef_raw` / `%pydef_raw`).
...
* `%rhaidef_raw` / `%pydef_raw` for computed logic
* `%env` only when explicitly enabled
* `%rhaidef` / `%pydef` (non-raw) when body preprocessing is intentional
...
* `%rhaidef_raw` / `%pydef_raw` raw-body script variants
...
* Multiple-`%here` error
* Deprecation of non-raw `%rhaidef` / `%pydef` in docs
