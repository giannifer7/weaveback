...
| `%{ … %}` / `%tag{ … %tag}` | Quoted argument block (single argument, still macro-active).
...
Quoted argument blocks have **no scope of their own**. They are evaluated in the enclosing
context.
...
| `body` | Last argument. Typically a quoted argument block `%{ … %}` when the body contains commas or parentheses.
|===
...
* `%here` is a source-rewrite operation, not a normal expression-forming macro.
  Generated code should not contain it. Treat it as operational machinery, not
  part of the calm conceptual core of the language.
