...
| `%convert_case(s, style)` | Dispatch by `style` string (`snake`, `camel`, `pascal`, `screaming`) | —
|===
...
If that path expands to empty, the include is skipped normally. If it expands
to a real path, the resolved path is recorded in `discovered_includes` and the
target file is not evaluated.
