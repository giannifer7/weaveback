...
* **Macro definitions** — `%def`, `%pydef`, `%pydef_raw`, `%alias`
...
`%pydef_raw` for new scripts to avoid this.

=== `%pydef_raw(name, [params…,] body)`

Like `%pydef`, but the body is stored as **literal script source
