...
This enables the https://en.wikipedia.org/wiki/X_macro[X macro] idiom: define
a list macro that calls a configurable inner macro `X` for each entry, then
rebind `X` before each use.
...
This allows tools like `wb-query trace` and the MCP server to pinpoint the 
