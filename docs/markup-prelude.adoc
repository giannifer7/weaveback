...
Use a macro-active body (`¤{ ... ¤}`) when table cells contain other prelude
components such as `¤link` or `¤xref`; use a verbatim body (`¤[ ... ¤]`) only
when the table body must remain fully literal.
...
`¤link(target, label)` and `¤xref(target, label)` preserve the target exactly as
written.  They do not infer extensions, rewrite paths, or decide whether a
document should be `.adoc`, `.md`, or `.html`.  Use `¤link` for ordinary URL or
file links and `¤xref` when the author intends an intra-document or
cross-document reference.  The prelude only projects that explicit intent to
the target markup syntax.
