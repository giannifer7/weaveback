...
`weaveback mcp` starts a [Model Context Protocol](https://modelcontextprotocol.io/)
...
1. **Locate the source**: call `weaveback_trace` with the generated file and line number.
2. **Read the source**: open `src_file` at `src_line` (and surrounding context).
3. **Apply the fix**: call `weaveback_apply_fix` with the replacement text and the
...
`weaveback_apply_fix` works for both single-line replacements (`src_line` only) and
