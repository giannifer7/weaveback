...
Pick a low-risk document that generates one small Rust file and has few custom
macros.

Conversion steps:

1. Copy `foo.adoc` to `foo.wvb`.
2. Replace only structural markup with `¤` macros.
3. Preserve prose as plain text where possible.
4. Configure two projections for that directory: one AsciiDoc, one Markdown.
5. Generate `expanded-adoc/foo.adoc` and `expanded-md/foo.md`.
6. Verify both projections tangle to byte-identical generated source.
7. Keep the original `.adoc` until the `.wvb` path is stable.
...
* keep `// <[chunk]>=` and `// @` visually on their own lines;
...
The first real migration should be a small Rust leaf module with:
...
The purpose of the first migration is to validate the workflow, not to maximize
coverage.
