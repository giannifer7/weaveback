...
literate intent into a small currency-sign macro layer, then project that intent to
...
* The currency-sign pass is macro-only. It should not write final source files directly.
...
Links remain explicit. The `link` and `xref` helpers do not
guess target extensions or rewrite paths; they only render the same author
target in the selected markup. This keeps `.wvb` neutral without hiding the
actual reference being made.

Table bodies that contain nested prelude macros must use macro-active blocks:
the `table` helper must receive a macro-active body block. Verbatim table bodies
...
4. Replace only structural markup with the currency-sign macro layer.
...
* Whether `.wvb` should allow raw AsciiDoc/Markdown sections, or whether all
  markup-specific syntax should be isolated behind the currency-sign macros.
