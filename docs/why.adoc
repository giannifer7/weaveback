...
programmers reach for Rhai (Rust-like syntax, pure Rust, no external dependency).
Python-familiar teams reach for `%pydef`, backed by
https://github.com/pydantic/monty[monty] — a pure-Rust Python interpreter that
...
`weaveback trace`**.  This is not a consequence of sandboxing — it is an
inherent property of dynamic code generation: the source map can point to the
`%rhaidef` call site, but there is no static source line inside the Rhai body
