...
Blocks are found by recursively walking the ACDC block tree.  For each
delimited listing/pass block whose language resolves to `"plantuml"`, we
record. This accepts both `[plantuml]` and `[source,plantuml]`.
