...
| `%def`, `%redef`, `%pydef`, `%pydef_raw`  | Define or rebind a macro (plain or Python)
| `%set`                         | Set a variable
...
`%pydef` and `%pydef_raw` calls and tells the editor to apply the `python`
grammar to its text content.
...
block body with Python highlighting regardless.

NOTE: monty (the Python back-end) is a sandboxed runtime with no filesystem
or network access. Language injection here is purely for visual benefit; it
